...
func _try_broadcast_player_stats() -> void:
	...
	# The active pet's display name (custom name if the player renamed it, else
	# the default) so remote players can render the right label on their copy.
	var pet_name: String = PetManager.get_pet_display_name(pet_id) if not pet_id.is_empty() else ""
	rpc("_receive_player_stats", health, MAX_HEALTH, hunger, MAX_HUNGER, player_name, pet_id, interior, in_mine, armor, lvl, held_item_id, in_building, inside_island_seed, downed_frac, current_building_key, pet_name)


@rpc("unreliable", "any_peer")
func _receive_player_stats(hp: int, max_hp: int, hgr: int, max_hgr: int, name: String, pet_id: String = "", interior: int = 0, in_mine: int = 0, armor_set: String = "", level: int = 1, held_item_id: String = "", in_building: int = 0, island_seed: int = 0, downed_frac: float = 0.0, building_key: String = "", pet_name: String = "") -> void:
