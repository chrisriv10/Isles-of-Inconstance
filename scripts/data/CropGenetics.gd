...
static func deserialize(data: Dictionary) -> CropGenetics:
	...
	# JSON deserializes arrays as untyped Array — must copy element-by-element
	# into typed Array[String] properties to avoid type mismatch errors.
	for tag: Variant in data.get("appearance_tags", []):
		genetics.appearance_tags.append(str(tag))
	for ability: Variant in data.get("special_abilities", []):
		genetics.special_abilities.append(str(ability))
	for ancestor: Variant in data.get("lineage", []):
		genetics.lineage.append(str(ancestor))
