...
func render(image: Image, center_x: int, center_y: int) -> void:
	match flower_type:
		FlowerType.SIMPLE:
			_draw_simple_flower(image, center_x, center_y)
		FlowerType.DAISY:
			_draw_daisy(image, center_x, center_y)
		FlowerType.TULIP:
			_draw_tulip(image, center_x, center_y)
		FlowerType.ROSE:
			_draw_rose(image, center_x, center_y)
		FlowerType.SPIKES:
			_draw_spikes(image, center_x, center_y)

func _draw_simple_flower(image: Image, x: int, y: int) -> void:
	# Draw petals
...
func _draw_daisy(image: Image, x: int, y: int) -> void:
	# Draw long petals
	for i in range(petal_count):
		var angle := deg_to_rad(float(i) * 360.0 / float(petal_count))
...
func _draw_spikes(image: Image, x: int, y: int) -> void:
	for i in range(petal_count):
		var angle := deg_to_rad(float(i) * 360.0 / float(petal_count))
