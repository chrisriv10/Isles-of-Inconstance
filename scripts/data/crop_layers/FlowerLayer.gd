...
func _draw_simple_flower(image: Image, x: int, y: int, _center_x: int, _center_y: int) -> void:
	# Draw petals
...
func _draw_daisy(image: Image, x: int, y: int, _center_x: int, _center_y: int) -> void:
	# Draw long petals
	for i in range(petal_count):
		var angle := deg_to_rad(float(i) * 360.0 / float(petal_count))
...
func _draw_tulip(image: Image, x: int, y: int, center_x: int, center_y: int) -> void:
	...
	for i in range(3):
		var petal_offset_x: int = (i - 1) * half_size
		for py in range(-half_size, half_size + 1):
			...
			for px in range(-int(width_at_y), int(width_at_y) + 1):
				var draw_x: int = x + petal_offset_x + px
...
func _draw_rose(image: Image, x: int, y: int, _center_x: int, _center_y: int) -> void:
	var layers := 3
...
func _draw_spikes(image: Image, x: int, y: int, _center_x: int, _center_y: int) -> void:
	for i in range(petal_count):
		var angle := deg_to_rad(float(i) * 360.0 / float(petal_count))
