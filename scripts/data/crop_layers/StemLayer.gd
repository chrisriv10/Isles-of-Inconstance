...
enum StemType { STRAIGHT, CURVED, VINE, MUSHROOM_STALK, CORN_STALK }
...
func render(image: Image, center_x: int, center_y: int) -> void:
	...
	match stem_type:
		...
		StemType.CORN_STALK:
			_draw_corn_stalk(image, center_x, start_y, end_y, center_x, center_y)
...
func _draw_corn_stalk(image: Image, x: int, start_y: int, end_y: int, center_x: int, center_y: int) -> void:
	# A thick, straight stalk with occasional node-knots
	var half_width := width / 2
	for y in range(end_y, start_y + 1):
		for dx in range(-half_width, half_width + 1):
			var draw_x: int = x + dx
			...
			var transformed := apply_transforms(draw_x, draw_y, center_x, center_y)
			PixelArtUtils.set_pixel(image, transformed.x, transformed.y, color)
		# Node knots: slightly wider every 3 rows
		if y % 3 == 0:
			for dx in [-half_width - 1, half_width + 1]:
				var knot := apply_transforms(x + dx, y, center_x, center_y)
				PixelArtUtils.set_pixel(image, knot.x, knot.y, Color(0.5, 0.4, 0.2))
