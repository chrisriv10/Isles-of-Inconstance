...
func _draw_circle(image: Image, x: int, y: int, center_x: int, center_y: int) -> void:
	var radius := size / 2
	PixelArtUtils.draw_circle(image, x, y, radius, color)

func _draw_oval(image: Image, x: int, y: int, center_x: int, center_y: int) -> void:
...
func _draw_star(image: Image, x: int, y: int, center_x: int, center_y: int) -> void:
	var half_size := size / 2
	var points: Array[Vector2i] = []
	for i in range(5):
