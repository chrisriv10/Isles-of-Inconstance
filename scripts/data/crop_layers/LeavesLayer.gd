...
enum LeafType { SIMPLE, POINTED, ROUNDED, FERN, VINE_LEAVES, CORN_LEAF, BROAD_LEAF }
...
func render(image: Image, center_x: int, center_y: int) -> void:
	for i in range(leaf_count):
		...
		match leaf_type:
			...
			LeafType.CORN_LEAF:
				_draw_corn_leaf(image, leaf_center_x, leaf_center_y, angle, center_x, center_y)
			LeafType.BROAD_LEAF:
				_draw_broad_leaf(image, leaf_center_x, leaf_center_y, angle, center_x, center_y)

func _draw_simple_leaf(image: Image, x: int, y: int, _angle: float, center_x: int, center_y: int) -> void:
...
func _draw_pointed_leaf(image: Image, x: int, y: int, _angle: float, center_x: int, center_y: int) -> void:
	var half_size := leaf_size / 2
	for dy in range(-half_size, half_size + 1):
...
func _draw_rounded_leaf(image: Image, x: int, y: int, _angle: float, _center_x: int, _center_y: int) -> void:
	var radius := leaf_size / 2
	PixelArtUtils.draw_circle(image, x, y, radius, color)

func _draw_fern_leaf(image: Image, x: int, y: int, _angle: float, center_x: int, center_y: int) -> void:
...
func _draw_vine_leaves(image: Image, x: int, y: int, _angle: float, center_x: int, center_y: int) -> void:
...
func _draw_corn_leaf(image: Image, x: int, y: int, _angle: float, center_x: int, center_y: int) -> void:
	# A single long curving leaf blade arching outward
	var length := leaf_size + 2
	for i in range(length):
		var progress := float(i) / float(length)
		var arch := int(2.0 * sin(progress * PI))  # arch upward then back
		var lx := x + i - length / 2
		var ly := y + arch - i / 2
		var transformed := apply_transforms(lx, ly, center_x, center_y)
		PixelArtUtils.set_pixel(image, transformed.x, transformed.y, color)

func _draw_broad_leaf(image: Image, x: int, y: int, _angle: float, center_x: int, center_y: int) -> void:
	# A teardrop-shaped broad leaf with a center vein
	var half_size := leaf_size / 2
	for dy in range(-half_size, half_size + 1):
		var progress := float(dy + half_size) / float(leaf_size)
		var width_at_y: float = half_size - abs(dy) / 2.0 + 1
		for dx in range(-width_at_y, width_at_y + 1):
			var draw_x: int = x + dx
			var draw_y: int = y + dy
			var transformed := apply_transforms(draw_x, draw_y, center_x, center_y)
			PixelArtUtils.set_pixel(image, transformed.x, transformed.y, color)
		# Center vein line (slightly brighter)
		if dy % 2 == 0:
			var vein := apply_transforms(x, y + dy, center_x, center_y)
			PixelArtUtils.set_pixel(image, vein.x, vein.y, Color(0.5, 0.8, 0.3))
