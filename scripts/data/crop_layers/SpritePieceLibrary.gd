...
static func get_leaf_pieces() -> Dictionary:
	return {
		...
		"corn_leaf": _create_corn_leaf(),
		"broad_leaf": _create_broad_leaf(),
	}
...
static func get_fruit_pieces() -> Dictionary:
	return {
		...
		"carrot": _create_carrot_fruit(),
		"corn_cob": _create_corn_cob_fruit(),
		"tomato": _create_tomato_fruit(),
		"pumpkin": _create_pumpkin_fruit(),
		"inconstant_fruit": _create_inconstant_fruit(),
	}
...
# ============================================================
# New recognizable crop shape pieces
# ============================================================

# --- Stem ---
static func _create_corn_stalk() -> StemLayer:
	var stem := StemLayer.new()
	stem.stem_type = StemLayer.StemType.CORN_STALK
	stem.height = 12
	stem.width = 4
	stem.color = Color(0.5, 0.6, 0.2)
	return stem

# --- Leaves ---
static func _create_corn_leaf() -> LeavesLayer:
	var leaves := LeavesLayer.new()
	leaves.leaf_type = LeavesLayer.LeafType.CORN_LEAF
	leaves.leaf_count = 4
	leaves.leaf_size = 5
	leaves.color = Color(0.4, 0.65, 0.2)
	return leaves

static func _create_broad_leaf() -> LeavesLayer:
	var leaves := LeavesLayer.new()
	leaves.leaf_type = LeavesLayer.LeafType.BROAD_LEAF
	leaves.leaf_count = 3
	leaves.leaf_size = 5
	leaves.color = Color(0.3, 0.6, 0.25)
	return leaves
...
# --- Fruit/Body ---
static func _create_carrot_fruit() -> FruitBodyLayer:
	var fruit := FruitBodyLayer.new()
	fruit.body_type = FruitBodyLayer.BodyType.CARROT
	fruit.size = 10
	fruit.color = Color(1.0, 0.5, 0.1)
	return fruit

static func _create_corn_cob_fruit() -> FruitBodyLayer:
	var fruit := FruitBodyLayer.new()
	fruit.body_type = FruitBodyLayer.BodyType.CORN_COB
	fruit.size = 10
	fruit.color = Color(0.95, 0.85, 0.3)
	return fruit

static func _create_tomato_fruit() -> FruitBodyLayer:
	var fruit := FruitBodyLayer.new()
	fruit.body_type = FruitBodyLayer.BodyType.TOMATO
	fruit.size = 10
	fruit.color = Color(0.9, 0.2, 0.15)
	return fruit

static func _create_pumpkin_fruit() -> FruitBodyLayer:
	var fruit := FruitBodyLayer.new()
	fruit.body_type = FruitBodyLayer.BodyType.PUMPKIN
	fruit.size = 12
	fruit.color = Color(0.95, 0.55, 0.1)
	return fruit

static func _create_inconstant_fruit() -> FruitBodyLayer:
	var fruit := FruitBodyLayer.new()
	fruit.body_type = FruitBodyLayer.BodyType.INCONSTANT_FRUIT
	fruit.size = 10
	fruit.color = Color(0.5, 0.3, 1.0)
	return fruit
