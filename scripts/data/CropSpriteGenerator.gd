...
func create_crop_from_pieces(stem_key: String, leaves_key: String, fruit_key: String, 
		...
		fruit_color: Color, flower_color: Color,
		stem_color: Color = Color(-1, -1, -1, -1),
		leaf_color: Color = Color(-1, -1, -1, -1)) -> CropSpriteGenerator:
	#
	# stem_color / leaf_color override the library's default colors for those
	# pieces. Pass a negative-alpha sentinel Color(-1,-1,-1,-1) to keep the
	# library default (which is the normal path for crop field textures).
