...
## Tracks the last crop harvested here for rotation bonus.
## When a different crop is planted, the soil gets a quality + growth boost.
var previous_crop_id: String = ""
...
func advance_daily() -> void:
    ...
    # Track days since crop was harvested / soil went empty
    if crop_id == "":
...
static func deserialize(data: Dictionary) -> SoilData:
    ...
    soil.previous_crop_id = data.get("previous_crop_id", "")
