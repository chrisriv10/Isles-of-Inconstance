...
func _generate_one() -> CropData:
	...
	var root_word: String = root_data.name
	...
	var tex_seed := _rng.seed + hash(crop_name)
	crop.growth_stage_textures = CropSpriteUtils.generate_crop_textures(crop_name, crop_color, tex_seed, root_word)
	...
	var yield_icon_seed := _rng.seed + hash("yield_" + crop_name)
	yield_item.icon = CropSpriteUtils.generate_seed_icon(crop_name, crop_color, yield_icon_seed, root_word)
	...
	var icon_seed := _rng.seed + hash("icon_" + crop_name)
	seed_item.icon = CropSpriteUtils.generate_seed_icon(crop_name, crop_color, icon_seed, root_word, true)
