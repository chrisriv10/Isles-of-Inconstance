...
## Yield bonus for crops planted in their preferred season.
## Returns 1.25x in-season, 1.0x otherwise.
func get_yield_multiplier(preference_tag: String) -> float:
	if preference_tag == "" or not is_season_favorable(preference_tag):
		return 1.0  # no yield penalty, just growth
	return 1.25  # in-season crops yield more
	
## Returns a descriptive string for the season effect on this crop.
func get_season_description(preference_tag: String) -> String:
	if is_season_favorable(preference_tag):
		var season_name := get_season_name()
		return "%s season — 1.25x growth & 1.25x yield!" % season_name
	else:
		return "Out of season — 0.75x growth (plant in preferred season for bonuses)"
