...
## Return a human-readable description of possible mutations for a crop.
## Shows which mutations are eligible based on the crop's root word.
func get_mutation_info(crop_id: String) -> String:
	var crop := DataManager.get_crop(crop_id)
	if not crop:
		return "Crop not found: " + crop_id
	
	var root_word := _extract_root_word(crop.display_name)
	...
	for m in _mutations:
		if not m.matches_root(root_word):
			continue
		var tier_str: String
		match m.min_rarity_tier:
			0: tier_str = "Any rarity"
			1: tier_str = "Uncommon+"
			2: tier_str = "Rare+"
			3: tier_str = "Epic+"
			_: tier_str = "Any rarity"
		var sell_bonus := int((m.sell_value_multiplier - 1.0) * 100)
		var bonus_str: String = ""
		if sell_bonus > 0:
			bonus_str = " (+%d%% value)" % sell_bonus
		lines.append("%s (%s)%s - %s" % [m.mutation_name, tier_str, bonus_str, m.description])
	
	if lines.is_empty():
		return "%s has no known possible mutations at this time." % crop.display_name
	
	return "Possible mutations for %s:\n%s" % [crop.display_name, "\n".join(lines)]
...
## `luck_bonus` (0.0-1.0+) adds to the base mutation_chance when a luck buff is active.
func roll_for_mutation(genetics: CropGenetics, rng: RandomNumberGenerator, luck_bonus: float = 0.0) -> bool:
	if genetics == null:
		return false
	var effective_chance: float = genetics.mutation_chance
	if luck_bonus > 0.0:
		# Luck buff adds its strength as bonus probability (e.g. 0.3 = +30% to mutation chance)
		effective_chance = clampf(effective_chance + luck_bonus, 0.0, 0.95)
...
func create_mutated_crop(base_crop: CropData, mutation: MutationDefinition,
	...
	var existing: CropData = DataManager.get_crop(new_id)
	if existing:
		return existing
	...
	var base_value := 14.0 * child_genetics.value_factor
	...
	var yield_icon_seed := hash("yield_" + new_name + str(rng.seed))
	var new_root := _extract_root_word(new_name)
	yield_item.icon = CropSpriteUtils.generate_seed_icon(new_name, child.modulate_color, yield_icon_seed, new_root)
	...
	var seed_icon_seed := hash("seed_" + new_name + str(rng.seed))
	seed_item.icon = CropSpriteUtils.generate_seed_icon(new_name, child.modulate_color, seed_icon_seed, new_root, true)
