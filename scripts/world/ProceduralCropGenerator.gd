...
func _generate_one(rng: RandomNumberGenerator) -> CropData:
	...
	var base_price  := floori(14.0 * value_mult)
	...
	# Generate a yield icon using the crop's color (different seed so it looks distinct from the seed icon)
	yield_item.icon = CropSpriteUtils.generate_seed_icon(crop_name, prefix.modulate_color, hash("yield_" + crop_name + str(rng.seed)))
...
func _pick_crop_pieces(seed_val: int, local_rng: RandomNumberGenerator, root_word: String = "") -> Dictionary:
	# Use the shared root template map if available
	if root_word != "" and CropSpriteUtils.ROOT_TEMPLATES.has(root_word):
		var tmpl: Dictionary = CropSpriteUtils.ROOT_TEMPLATES[root_word]
		return {
...
func _pick_crop_colors(prefix: CropTrait, _local_rng: RandomNumberGenerator) -> Dictionary:
	# Use a deterministic RNG seeded from the prefix color so seed icons and
	# crop textures always produce IDENTICAL fruit/flower colors from the same
	# base — guaranteeing visual relationship: a blue seed grows a blue crop.
	#
	# FRUIT uses the prefix color *directly* (no jitter) so the seed icon's
	...
	color_rng.seed = hash(prefix.modulate_color)
	var base_color := prefix.modulate_color
