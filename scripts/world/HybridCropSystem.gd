...
func try_hybridize(parent_a_id: String, parent_b_id: String, tile_x: int, tile_y: int) -> CropData:
	var crop_a: CropData = DataManager.get_crop(parent_a_id)
	var crop_b: CropData = DataManager.get_crop(parent_b_id)
	...
	# Average stats
	var avg_days := roundi((crop_a.days_to_grow + crop_b.days_to_grow) / 2.0)
	var avg_yield := roundi((crop_a.yield_amount + crop_b.yield_amount) / 2.0)
	...
	var tex_seed := rng.seed + hash(hybrid_name)
	var hybrid_root := hybrid_name.split(" ")[-1] if " " in hybrid_name else ""
	hybrid.growth_stage_textures = CropSpriteUtils.generate_crop_textures(hybrid_name, hybrid_color, tex_seed, hybrid_root)
	...
	var yield_icon_seed := rng.seed + hash("yield_" + hybrid_name)
	yield_item.icon = _generate_hybrid_yield_icon(hybrid_name, hybrid_color, yield_icon_seed)
	...
	var icon_seed := rng.seed + hash("icon_" + hybrid_name)
	seed_item.icon = CropSpriteUtils.generate_seed_icon(hybrid_name, hybrid_color, icon_seed, hybrid_root, true)
	DataManager.register_item(seed_item)
	
	return hybrid

## Generates a distinct icon for the hybrid yield item using CropSpriteUtils.
func _generate_hybrid_yield_icon(crop_name: String, crop_color: Color, seed_val: int) -> Texture2D:
	var root_word := crop_name.split(" ")[-1] if " " in crop_name else ""
	return CropSpriteUtils.generate_seed_icon(crop_name, crop_color, seed_val, root_word)

## Splits a crop display name (e.g. "Azure Melon") into [prefix, root]
func _split_crop_name(crop_name: String) -> Array:
	var parts := crop_name.split(" ", true, 1)
	if parts.size() >= 2:
