...
func _register_default_recipes() -> void:
	...
	# --- Bakery Recipes (breads and pastries) ---
	_add_recipe("fresh_bread", "Fresh Bread", "A warm crusty loaf. Simple and satisfying.",
		{"dough": 1, "wood": 1}, 15, "energy", 3.0, 0.3, "snack")
	_add_recipe("buttered_bread", "Buttered Bread", "Warm bread slathered in creamy butter.",
		{"bread": 1, "milk": 1}, 22, "energy", 4.0, 0.5, "snack")
	_add_recipe("cinnamon_toast", "Cinnamon Toast", "Toasted bread sprinkled with cinnamon and sugar.",
		{"bread": 1, "sugar_crystal": 1}, 25, "speed", 3.0, 0.25, "snack")
	_add_recipe("fruit_tart_bake", "Fruit Tart", "A golden pastry filled with fresh glazed fruit.",
		{"dough": 1, "crop": 3, "sugar_crystal": 1}, 28, "health", 5.0, 0.5, "dessert")
	_add_recipe("chocolate_croissant", "Chocolate Croissant", "A flaky croissant filled with rich chocolate.",
		{"croissant": 1, "chocolate_chunk": 1}, 35, "energy", 5.0, 0.6, "dessert")

	# ── Crafted Island Foods (upgraded to full meals with buffs) ──
	_add_recipe("frozen_delight", "Frozen Delight", "A chilly treat that refreshes and invigorates.",
		{"ice_crystal": 2, "berry": 1}, 30, "speed", 3.0, 0.15, "dessert")
	_add_recipe("chocolate_fondue", "Chocolate Fondue", "Rich melted chocolate that warms the soul.",
		{"chocolate_chunk": 2, "berry": 1}, 28, "energy", 4.0, 0.3, "dessert")
	_add_recipe("candied_cactus", "Candied Cactus", "Exotic candied cactus fruit. Brings good fortune!",
		{"cactus_fruit": 2, "sugar_crystal": 1}, 25, "luck", 3.0, 0.2, "snack")
	_add_recipe("sugar_sculpture", "Sugar Sculpture", "An artisan sugar masterpiece. A real energy boost!",
		{"sugar_crystal": 3, "chocolate_chunk": 2}, 100, "health", 5.0, 0.3, "dessert")

	# ── (Nut recipes removed — their result items don't exist yet) ──

func _add_recipe(id: String, recipe_name: String, desc: String, ingredients: Dictionary, price: int, buff: String, duration: float, strength: float, category: String) -> void:
	var meal := MealData.new()
	meal.id = id
	meal.display_name = recipe_name
