...
func _physics_process(delta: float) -> void:
	...
	if input_direction != Vector2.ZERO:
		...
		# _walk_bob(delta)  # Disabled — was causing "step by step" feeling
	else:
		_reset_walk_bob()
...
func _is_water_cell(_world_unused: Node, cell: Vector2i) -> bool:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("interact"):
		# If in build mode, place building instead of normal interaction
