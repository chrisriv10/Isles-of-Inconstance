...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("open_cooking"):
		# Can only cook when near a campfire or inside a building with a kitchen
		if not GameManager.near_campfire and not GameManager.inside_interior:
			ToastNotification.show_toast("Must be near a campfire or kitchen to cook!", ToastNotification.ToastType.INFO, 2.5)
			return
		toggle()
...
func refresh() -> void:
	...
	if not cs:
		info_label.text = "You need a cooking system. Try building a campfire!"
		return
	...
	for cat in category_order:
		...
		for meal in meals:
			...
			ing_label.custom_minimum_size = Vector2(180, 0)
...
func _on_cook_pressed(cs: CookingSystem, recipe_id: String) -> void:
	if cs.cook(recipe_id) > 0:
		...
		LevelManager.add_xp_source("cook")
		refresh()
	else:
